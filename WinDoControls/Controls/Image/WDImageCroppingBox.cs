...
{
    public class WDImageCroppingBox : System.Windows.Forms.Control
    {
        public WDImageCroppingBox()
        {
        ...
        public Image Image
        {
            ...
            set
            {
                ...
                if (value != null)
                    _Image = ImageHelper.resizeImage(value, new Size(this.Width, this.Height));
                this.Clear();
            }
        }
        ...
        protected override void OnCreateControl()
        {
            ...
            m_rectDots[7].Size = new Size(8, 8);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {//否则可能表示在选择区域或重置大小
                ...
                {//是否允许选区宽度改变 如重置大小时候 拉动上边和下边中点时候
                    ...
                    this._SelectedRectangle.Height = this._SelectedRectangle.Width = Math.Abs(e.Location.X - m_ptStart.X);
                }
                if (m_bChangeHeight)
                {
                    this._SelectedRectangle.Y = e.Location.Y > m_ptStart.Y ? m_ptStart.Y : e.Location.Y;
                    this._SelectedRectangle.Width = this._SelectedRectangle.Height = Math.Abs(e.Location.Y - m_ptStart.Y);
                }
                if (m_bMoving)
                {//如果是移动选区 判断选区移动范围
        ...
        Pen penBorder = new Pen(WDColors.OrangeColor) { Width = 2 };
        ...
        protected virtual void DrawSelectedRectangle(Graphics g)
        {
            ...
            var _9pen = new Pen(Color.White, 2.0f);
            //指定线条的样式为划线段
            _9pen.DashStyle = System.Drawing.Drawing2D.DashStyle.Dash;
            var point1 = new Point(this._SelectedRectangle.Width / 3, 0);
            point1.Offset(this._SelectedRectangle.Location);
            var point2 = new Point(point1.X, point1.Y + this._SelectedRectangle.Height);
            g.DrawLine(_9pen, point1, point2);
            point1.Offset(this._SelectedRectangle.Width / 3, 0);
            point2.Offset(this._SelectedRectangle.Width / 3, 0);
            g.DrawLine(_9pen, point1, point2);
            point1 = new Point(0, this._SelectedRectangle.Height / 3);
            point1.Offset(this._SelectedRectangle.Location);
            point2 = new Point(point1.X + this._SelectedRectangle.Width, point1.Y);
            g.DrawLine(_9pen, point1, point2);
            point1.Offset(0, this._SelectedRectangle.Height / 3);
            point2.Offset(0, this._SelectedRectangle.Height / 3);
            g.DrawLine(_9pen, point1, point2);
            g.DrawRectangle(penBorder, this._SelectedRectangle.Left, this._SelectedRectangle.Top, this._SelectedRectangle.Width - 1, this._SelectedRectangle.Height - 1);

            foreach (Rectangle rect in m_rectDots)
                g.FillRectangle(penBorder.Brush, rect);
        ...
        public virtual Image GetSelectedImage()
        {
            ...
            if (this._SelectedRectangle.Width == 0) return null;
            if (this._SelectedRectangle.Height == 0) return null;
